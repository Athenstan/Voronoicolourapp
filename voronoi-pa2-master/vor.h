...
vector<center> randSample(PNG& img, double density);
    ...
    bool isValid(int x, int y, int width, int height, center c, int k, vector<vector<bool>> processed);
    int getDist(int x, int y, center c);
    
    template <template <class T> class OrderingStructure>
    bool isEmpty(vector<OrderingStructure<point>> vect);
  
    vector<pair<int, int>> makeNeighDict();
}
